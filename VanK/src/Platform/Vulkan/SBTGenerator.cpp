...
// Default setup
//
void SBTGenerator::init(vk::raii::Device& device, const vk::PhysicalDeviceRayTracingPipelinePropertiesKHR& rayProperties)
{
...
//
void SBTGenerator::addIndices(vk::RayTracingPipelineCreateInfoKHR                         rayPipelineInfo,
...
//
size_t SBTGenerator::calculateSBTBufferSize(vk::raii::Pipeline&                                         rayPipeline,
                                            ...
                                            std::span<const vk::RayTracingPipelineCreateInfoKHR> librariesInfo)
{
  ...
  if(rayPipelineInfo.sType == vk::StructureType::eRayTracingPipelineCreateInfoKHR)
  {
    ...
    for(const vk::RayTracingPipelineCreateInfoKHR& lib : librariesInfo)
    {
  ...
  auto findStride = [&](shaderRecordMap& entries, uint32_t& stride) {
    stride = align_up(m_handleSize, m_handleAlignment);  // minimum stride is handle size aligned
    for(const std::pair<uint32_t, std::vector<uint8_t>>& entry : entries)
    {
      ...
      uint32_t dataHandleSize =
          align_up(static_cast<uint32_t>(m_handleSize + entry.second.size() * sizeof(uint8_t)), m_handleAlignment);
  ...
  totalSize += align_up(m_stride[eRaygen] * getGroupIndexCount(eRaygen), bufferAlignment);
  m_bufferAddresses[eMiss] = totalSize;
  totalSize += align_up(m_stride[eMiss] * getGroupIndexCount(eMiss), bufferAlignment);
  m_bufferAddresses[eHit] = totalSize;
  totalSize += align_up(m_stride[eHit] * getGroupIndexCount(eHit), bufferAlignment);
  m_bufferAddresses[eCallable] = totalSize;
  totalSize += align_up(m_stride[eCallable] * getGroupIndexCount(eCallable), bufferAlignment);
...
//
vk::Result SBTGenerator::populateSBTBuffer(vk::DeviceAddress bufferAddress, size_t bufferSize, void* bufferData)
{
  ...
  /*VanK_FAIL_RETURN(vkGetRayTracingShaderGroupHandlesKHR(m_device, m_pipeline, 0, m_totalGroupCount, sbtSize,
                                                        shaderHandleStorage.data()));*/
  
  shaderHandleStorage = m_pipeline->getRayTracingShaderGroupHandlesKHR<uint8_t>(0, m_totalGroupCount, sbtSize);

  // Write the handles in the SBT buffer + data info (if any)
  auto copyHandles = [&](vk::DeviceAddress offset, std::vector<uint32_t>& indices, uint32_t stride, shaderRecordMap& data) {
...
vk::DeviceAddress SBTGenerator::getGroupAddress(GroupType t) const
{
  assert(m_bufferAddresses[t]);
  return m_bufferAddresses[t];
}

const vk::StridedDeviceAddressRegionKHR SBTGenerator::getSBTRegion(GroupType t, uint32_t indexOffset) const
{
  return vk::StridedDeviceAddressRegionKHR{getGroupAddress(t) + indexOffset * getGroupStride(t), getGroupStride(t), getSize(t)};
}
